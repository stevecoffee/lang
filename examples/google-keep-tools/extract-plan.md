...
Kind: collection (file `extract-plan.md` + directory `extract-plan/` — does not compile to a code file)
...
- Collection file is `extract-plan.md` (sibling of directory `extract-plan/`); children are only inside `extract-plan/`.
